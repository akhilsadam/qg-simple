...
def circular(t, grid, r, tolerance=1e-3, **kwargs):
    ...
    return mask # Move to the correct device (same as grid)

####################################################################################################

valid_mask = lambda _mask: isinstance(_mask, dict) and 'function' in _mask and _mask['function'] in mask_library

mask_library = {
    'circular': circular,
}

solve_mask = lambda _mask: mask_library[_mask['function']](**_mask) if valid_mask(_mask) else _mask